

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="java.io.*" %>
<%@page import="java.util.*"%>
<%@page import="java.lang.*"%>
<!DOCTYPE html>
<html>
    <head>
        <link rel="stylesheet" type="text/css" href="../css/stylep.css" />
        <link rel="stylesheet" type="text/css" href="/AplicativoZComunicaciones/faces/jsfcrud.css" />
	<script type="text/javascript" src="../js/jquery-1.4.2.min.js"></script>
	<script src="../js/jquery.autocomplete.js"></script>	
	
	<script>
	$(function(){
		$("#Cliente").autocomplete("../BusquedaCliente.jsp");
	});
         </script>
	  <%!      
        String nombreCliente;
       
        %>
</head>
<body style="background-image: url(../img/navidad3.jpg); background-repeat: no-repeat">
<br><br>
<font face="verdana" size="2">

	
	
</font>
 <div id="BuscarCliente">
        <table>  
             <form action="/AplicativoZComunicaciones/faces/clientes/New.jsp">
                  <tbody>
                <tr>
                <td width="62" height="27" ><a style="color:#fff"> NOMBRE CLIENTE</a></td>
                </tr>
                <tr>
                    <td width="193"><input  maxlength="11" type="Text" id="Cliente" name="Cliente" class="Cliente" onclick="" style="width: 520px ; height: 25px" />
                  </td>
                </tr>
                <tr>
                     <td height="30"><input type="submit" value="Siquiente"  class="boton"/></td>
                </tr>
                </tbody>
            </form>
        </table>
               </div>
 
<% 
      String  nombreC=request.getParameter("Cliente");
      session.setAttribute("nombreCli", nombreC);
%>
    
</body>
</html>
